...
def step_impl(content, sQuestion, sAnswer):
    ...
    if sAnswer.upper() == 'ALLOW':
        finger.f_AndroidAnswerToOSPermission(content.appiumSession, sQuestion ,android_os_permission_button.PERMISSION_ALLOW)
    elif sAnswer.upper() == 'DENY':
        finger.f_AndroidAnswerToOSPermission(content.appiumSession, sQuestion ,android_os_permission_button.PERMISSION_DENY)
    else:
...
@when("we tap on ImageButton {sProperties}:{sDescription}")
def step_impl(content, sProperties ,sDescription):
    finger.f_TapWidgetByPropertiesAndValue(content.appiumSession, 'android.widget.ImageButton', sProperties ,sDescription)
    pass

# @when('we tap on "{sWidget}":"{sProperties}":"{sDescription}"')
# def step_impl(content, sWidget, sProperties ,sDescription):
#     finger.f_TapWidgetByPropertiesAndValue(content.appiumSession, sWidget, sProperties ,sDescription)
#     pass


@when('we tap on specific location "{iX:d}":"{iY:d}"')
...
def step_impl(content, sWidget, sProp, sTarget, dPadKey):
    ...
    while not(bFound) or bRetry:
        els = finger.f_FindTargetByXPath(content.appiumSession, sWidget, sProp, sTarget)
        if els:
            bFound=True
            bRetry=False
        else:
